 ...
 */
  public static double totalSentiment(String filename)
  {
    ...
    int spaceIndex = fullReview.indexOf(" ");
    int i = 0;
    // loop through the file contents 
    while(i < fullReview.length()) {
      spaceIndex = fullReview.indexOf(" ", i);
      if (spaceIndex == -1) {
        spaceIndex = fullReview.length() - 1;
      }         
      String textWord = fullReview.substring(i, spaceIndex);
      String cleanWord = removePunctuation(textWord);
      sentimentTotal += sentimentVal(cleanWord);
      i += textWord.length() + 1;
    }
       // find each word
